...
function contentInterface($){
     ...
     let contentInterface = jQuery(tweak_bb.page_id +" > "+tweak_bb.row_element).find(".item h3").filter(':contains("Content Interface")').eq(0);
     // Find any Word Document link that's been added
     let wordDoc = jQuery(tweak_bb.page_id +" > "+tweak_bb.row_element).find(".item h3").filter(':contains("Content Document")').eq(0);
     
     [TERM, YEAR] = calculateTerm();
...
//          let title = jQuery(this).text();
     ...
     let icons = jQuery(".accordion").accordion("option", "icons");
     ...
     let journey = jQuery(contentInterface).parent().next('div.details').children('.vtbegenerated');    
     let child = jQuery(journey).children("#html");    
     jQuery(child).unwrap();
 }
 ...
 function handleFootNotes() {
     ...
     let footnotes = jQuery('#GU_ContentInterface a[id^="footnote-ref-"');
     let firstFootNote = '';
 
     footnotes.each( function() {
         // get the <sup> item wrapped around footnote
         let supItem = jQuery(this).parent();
 
         // get the id for the footnote content (at end of doc)
         //  footnote-ref-??  becomes
         //  footnote-??
         let footnoteId = jQuery(this).attr("id");
         let footnote = "li# " + footnoteId.replace('-ref','');
         footnote = footnote.replace(' ', '');
         let footnoteContent = jQuery( footnote).html();
         ...
         footnoteContent = footnoteContent.replace( footnote_re, '');
 
         // need to remove the link on the footnote reference
         let refHtml = jQuery(this).html();
     ...
     if (footnotes.length) {
         ...
         let footNoteList = jQuery( firstFootNote ).parent();
         jQuery(footNoteList).before("<h1>Footnotes</h1>");
         //remove the return anchor TODO replace it with something that works
         let footNoteListHtml = jQuery(footNoteList).html().replace( footnote_re, '');
 ...
 function setUpEdit(ci, params) {
   ...
   if ( typeof path === 'undefined'){
     ...
     let html = UPDATE_HTML + WORD_DOC_NOT_PRESENT;
 
     // add in link to edit the content item
     let editContent = 'into the <a href="https://bblearn-blaed.griffith.edu.au/webapps/blackboard/execute/manageCourseItem?content_id=' + contentId + '&course_id=' + courseId + '&dispatch=edit">Content Interface content item</a>';
   ...
   let updateHtml = UPDATE_HTML + WORD_DOC_PRESENT + INSTRUCTIONS;
 ...
 function checkParams( contentInterface,wordDoc) {
     ...
     let cssURL = DEFAULT_CSS;
     
     // Check parameters in the Content Interface item title
     if (contentInterface.length>0) {
         let contentInterfaceTitle = jQuery.trim(contentInterface.text());
         
         let m = contentInterfaceTitle.match(/content interface\s*([^<]+)/i );
 ...
 function createBunchesCards( bunch, index, arr) {
     ...
     let cards = extractCardsFromContent( bbItems);
 ...
 function handleBlackboardContentLink() {
     let hidden_string = " (not currently available)";
     let inner = false; // indicates whether the link is insider or outer
    // return false;
     // get the title from the Blackboard Item Heading (2)
     ...
     /* Find the matching Blackboard element heading (h3) */
     let bbItem = jQuery(tweak_bb.page_id +" > "+tweak_bb.row_element).find("h3:textEquals(" + title +")");
 ...
 function handleDate( description ) {
     ...
     date = { start: empty1, stop: empty2 } ; // object to return 
     ...
     //        console.log('ZZZZZZZZZZZZZZZZZZZZZZ handling a range');
             week = x[1];
             endWeek = x[2];
             date.stop = getTermDate( TERM, endWeek, false);
       ...
       //      console.log('ZZZZ week date, but not a range');
         ...
         date.start = getTermDate( TERM, week );
     } else {
         ...
         if (m) {
             ...
             if (m) {
                 ...
                 date.start = getTermDate( TERM, 'exam');
                 date.stop = getTermDate(TERM, 'exam', false);
                 description = description.replace( "<p>"+m[0]+"</p>","");
                 description = description.replace(m[0],"");
...
var HIDDEN_FROM_STUDENTS = `<div class="inline-block bg-yellow text-black text-xs rounded-t rounded-b">This item is <strong>hidden from students</strong></div>`;
